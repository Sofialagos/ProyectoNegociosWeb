...
<section>
  <form action="index.php?page=mnt_venta" method="post">
    ...
    <fieldset>
     ...
     <fieldset>
      <label for="productoVendido">Codigo Producto</label>
      <input {{if readonly}}readonly{{endif readonly}} type="text" id="productoVendido" name="productoVendido" placeholder="id producto" value="{{productoVendido}}" />
      {{if error_productoVendido}}
          {{foreach error_productoVendido}}
            <div class="error">{{this}}</div>
          {{endfor error_productoVendido}}
      {{endif error_productoVendido}}
    </fieldset>
